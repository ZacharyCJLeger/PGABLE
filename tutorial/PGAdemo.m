...
disp('(The important background that you need from GAdemo are outer, inner,');
...
disp('e0.e0=0 even though e0 is not the zero vector.  You can test this now');
disp('in PGA by typing   inner(e0,e0)  and   e0==0');
...
disp('The Euclidean part represents is the bivector perpendicular to the ');
...
disp('Euclidean part e1^e2^e3, and an e0 part.  The Euclidean bivector');
...
disp('coordinates 0.25 z, 0.5 y, and 1 x (the minus signs appear in the');
disp('equation because of the basis we have chosen to display things).');
...
disp('The e0 componenet of the earlier line was an offset from the origin');
disp('to the closest point on the line.  For nv, the e0 component gives');
disp('the direction to the line at infinity.');
disp(' ');
disp('In the graphics window, it is unclear how to draw a line at infinity.');
disp('We have chosen to draw nv as a dashed line around the Matlab drawing ');
disp('volume.  Technically, where this line would appear depends on where');
disp('you view it from; we have chosen to draw this line as if viewed from');
disp('the center of the Matlab drawing volume.');

disp(' ');GAprompt;disp(' ');

clc
disp('We intersect a line at infinity, with another plane, giving us a');
disp('');
disp('we refer to as a vanishing line.  Let''s clear the screen and draw two');
disp('parallel planes:'); disp(' ');

disp(' ');GAprompt;disp(' ');
disp('This is the end of our brief introduction to PGA in PGABLE.');
disp('For a further discussion on PGA, see our tutorial.');
catch ; end
